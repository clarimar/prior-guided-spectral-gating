...
training size studied. Its contribution is interpretive: gates initialised from the prior stay
chemically coherent ($\rho(\mathbf{g},\mathbf{s}) \geq 0.9987$) and select a
reproducible band set across seeds (Jaccard $0.945$--$0.987$), whereas
uninformed gates of equal accuracy do neither ($\rho \leq 0.28$, Jaccard
$0.09$--$0.57$). PGSGv2 provides a
...
(v)~assessment of four interpretability metrics (hypotheses H1--H4),
evaluated under both literature-informed and uninformed initialization,
which dissociate predictive performance from the reproducibility and
chemical coherence of the learned gate.
...
Tables~\ref{tab:performance} and~\ref{tab:interpret}, together with
Figure~\ref{fig:hypotheses}, summarize the four metrics across the $n$-grid.
We report performance and interpretability separately because the revised
analysis dissociates them: the two initializations are equivalent in the
former and sharply different in the latter.
...
(Table~\ref{tab:performance}). The only nominally significant value occurs at
...
With the literature prior, $\rho(\mathbf{g},\mathbf{s}) \geq 0.9987$ at every
training size (Table~\ref{tab:interpret}), decreasing slightly and steadily
as $n$ grows, from $1.0000$ at $n \leq 100$ to $0.9987$ at $n = 1{,}159$. The
learned gate therefore departs from its initialization only marginally, even
when abundant data are available. Combined with non-trivial gate standard
deviations ($\sigma(\mathbf{g}) = 0.12$--$0.18$), this should not be read as a
convergence failure: the gate is trainable, as the gradient analysis of
Section~5.1 establishes, but the objective offers little incentive to move
away from a chemically plausible starting point.

The uninformed condition makes the contrast concrete. Starting from
$\theta_0 = \mathbf{0}$, the converged gate reaches only $\rho = 0.035$ at
$n = 30$ and $\rho = 0.280$ at $n = 1{,}159$: it never approaches the
literature profile, yet its predictive accuracy is statistically
indistinguishable throughout (Table~\ref{tab:performance}). The optimization
problem thus admits solutions of equivalent accuracy whose spectral
weightings are almost unrelated to one another, and the prior selects among
them rather than improving upon them. This also confirms the absence of
VIP-style circularity: the two conditions share architecture and training
procedure yet converge to profiles that barely overlap.

The trend in the uninformed condition is in the expected direction --- $\rho$
rises from $0.035$ to $0.280$ as $n$ grows --- though not monotonically, with
a dip at $n = 60$ and a plateau between $n = 200$ and $n = 400$. More data do
pull the gate towards chemically relevant regions, but slowly: at the largest
training size available here the uninformed gate still shares little
structure with the literature profile.

\paragraph{H4 (Gate stability).}
Jaccard overlap of the top-decile gate indices across seed pairs ranges from
$0.945$ to $0.987$ under the literature prior, with no consistent trend in
$n$. Under uninformed initialization it ranges from $0.090$ to $0.566$
(Table~\ref{tab:interpret}). The prior thus multiplies the reproducibility of
the selected band set by a factor of between $1.7$ and $10.6$, largest where
data are scarcest.

This dissociation is, in our view, the central result of the revised
analysis. At every training size the two conditions predict equally well; at
every training size only one of them yields an explanation that survives a
change of random seed. A gate that selects a different tenth of the spectrum
on each run cannot be offered to a spectroscopist as a feature-importance
profile, however accurate its predictions. The contribution of a
literature-derived prior is therefore not to make the model more accurate,
but to make its interpretation stable enough to be worth reading --- which
is, for a method whose stated purpose is interpretable calibration, the
property that matters.

We note that the same dissociation is reported, on a different modality,
target analyte and instrument, in a companion study of cross-modality
generalization to Raman spectroscopy: statistical equivalence in accuracy,
with pairwise Jaccard of $0.90$--$0.94$ under a literature prior versus
$0.58$--$0.65$ without one.

\begin{table}[htb]
  \centering
  \caption{Performance metrics per training size $n$.
           ...
           Wilcoxon signed-rank test over ten seeds. No value of
  ...
  \begin{tabular}{r r c r}
    ...
    \midrule
      30 & $+0.614$ & $+0.010 \pm 0.078$ & $0.625$ \\
      60 & $+0.205$ & $-0.013 \pm 0.045$ & $0.322$ \\
     100 & $-0.091$ & $-0.046 \pm 0.060$ & $0.037$ \\
     200 & $-0.014$ & $+0.027 \pm 0.051$ & $0.193$ \\
     400 & $+0.057$ & $+0.011 \pm 0.051$ & $0.695$ \\
     700 & $+0.140$ & $+0.019 \pm 0.028$ & $0.084$ \\
    1159 & $+0.218$ & $-0.005 \pm 0.013$ & $0.770$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[htb]
  \centering
  \caption{Interpretability metrics per training size $n$, for both
           initializations, over ten seeds.
           $\rho(\mathbf{g},\mathbf{s})$: Pearson correlation between the
           learned gate and the literature prior. Jaccard: mean overlap of
           the top-decile bands ($q = 28$ of $p = 281$) across all pairs of
           seeds. The two conditions are statistically indistinguishable in
           accuracy (Table~\ref{tab:performance}) yet differ sharply here:
           without a prior the converged gate bears almost no relation to
           known absorption chemistry and selects different bands on each
           run.}
  \label{tab:interpret}
  \renewcommand{\arraystretch}{1.15}
  \begin{tabular}{r cc cc}
    ...
    $n$ & $\rho(\mathbf{g},\mathbf{s})$ & Jaccard
...
\begin{figure}[htb]
  ...
  \caption{Interpretability metrics as a function of training size $n$.
           ...
           shows a significant advantage for the literature prior.
           (c)~H3 and (d)~H4 contrast the two initializations: the
           literature-initialized gate stays close to the prior and selects
           a reproducible band set at every sample size, whereas the
           uninformed gate does neither, despite equivalent accuracy.}
...
improvement over PLS and 5\% over a standard MLP---with gate stability
(Jaccard $0.945$--$0.987$) and chemical coherence
($\rho(\mathbf{g}, \mathbf{s}) \geq 0.9987$) across random seeds. In the
...
sample size. Its role is revealed instead by the interpretability metrics:
an uninformed gate of statistically indistinguishable accuracy attains
$\rho \leq 0.28$ against the literature profile and a cross-seed Jaccard as
low as $0.09$, meaning that the same predictive performance is reached by
weighting spectral regions that bear little relation to known absorption
chemistry and that change from run to run. The prior selects, among many
equally accurate solutions, the one that can be read.
