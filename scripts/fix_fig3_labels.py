"""
Figura 3 - Labels dos métodos AO LADO dos círculos (sem sobreposição)
"""
...
print("="*70)
print("FIXING FIGURE 3: Labels beside circles (no overlap)")
print("="*70)

fig, ax = plt.subplots(figsize=(9, 5))  # Ligeiramente mais larga
...
# Plotar os círculos (APENAS os círculos, sem labels)
for i, (param, r2, color) in enumerate(zip(params, r2_scores, colors)):
    ax.scatter(param, r2, s=250, color=color, alpha=0.85, 
               edgecolor='black', linewidth=1.8, zorder=5)

# Adicionar labels AO LADO dos círculos (sem sobreposição)
# Estratégia: posicionar manualmente para evitar overlap

# PLSR - à direita do ponto
...
# BandSelectNet (Tecator) - acima do ponto
...
# BandSelectNet (Gasoline) - acima do ponto
...
print("\nChanges:")
print("  - PLS → PLSR ✓")
print("  - Labels positioned BESIDE circles (no overlap) ✓")
print("  - White boxes around labels for better readability ✓")
print("="*70)
